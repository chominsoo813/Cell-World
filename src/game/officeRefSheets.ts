...
export interface OfficeSheetConfig {
  ...
  /** Max CALC attainable on this sheet (recharge/cache ceiling). Defaults to 5. */
  maxCalc?: number;
  mission: string;
...
> = {
  1: [
    ...
    { title: "사라진 벽", workbook: "CONTRACT_ARCHIVE.xlsx", functionName: "HIDE", layout: "records", mission: "기록실을 통과해 계약서를 제출", maxCalc: 7 },
    ...
    { title: "수습 평가표", workbook: "PROBATION_REVIEW.xlsx", functionName: "HIDE + COPY", layout: "audit", mission: "평가 점수를 75로 복구해 제출", maxCalc: 7 },
  ],
  2: [
    ...
    { title: "조건부 검문소", workbook: "CLEARANCE_FILTER.xlsx", functionName: "FILTER", layout: "records", mission: "유효한 보안 등급만 남겨 검문소 통과", maxCalc: 7 },
    { title: "감사 대기열", workbook: "AUDIT_QUEUE.xlsx", functionName: "FILTER + SORT", layout: "meeting", mission: "팀장 업무를 우선순위대로 처리" },
    { title: "표본 감사", workbook: "SAMPLE_AUDIT.xlsx", functionName: "FILTER + SORT", layout: "audit", mission: "오늘의 정상 기록으로 감사 표본 구성", maxCalc: 7 },
  ],
  3: [
    { title: "문을 여는 직원", workbook: "EMPLOYEE_TRIGGER.xlsx", functionName: "IF", layout: "operations", mission: "직원 위치 조건으로 보안문 개방" },
    { title: "지금 참, 나중에 참", workbook: "DELAYED_TRUE.xlsx", functionName: "IF", layout: "records", mission: "즉시·지연 조건 두 개를 순서대로 만족", maxCalc: 7 },
    ...
    { title: "무한 업무 루프", workbook: "TASK_LOOP.xlsm", functionName: "IF + MACRO", layout: "audit", mission: "자동화를 중지하고 LOOP_DEPTH를 0으로 복구", maxCalc: 7 },
  ],
  4: [
    { title: "취소되지 않은 문", workbook: "CHANGE_HISTORY.xlsx", functionName: "UNDO", layout: "operations", mission: "복제 작업만 취소하고 열린 문 상태 보존" },
    { title: "마지막 수정자", workbook: "LAST_EDITOR.xlsx", functionName: "IF + PASTE", layout: "checkpoint", mission: "미끼 오브젝트로 감사 복구 대상 변경", maxCalc: 7 },
    { title: "겹쳐진 자리", workbook: "PREVIOUS_FLOORPLAN.xlsx", functionName: "IF + UNDO", layout: "records", mission: "과거 배치로 통로와 CCTV 엄폐 동시 확보", maxCalc: 7 },
  ...
  6: [
    ...
    { title: "저장되지 않은 변경", workbook: "UNSAVED_CHANGES.xlsx", functionName: "SAVE", layout: "meeting", mission: "저장 슬롯을 연결해 변경 사항 확정", maxCalc: 7 },
    { title: "통합 문서 보호 해제", workbook: "CELL_OFFICE_ROOT.xlsx", functionName: "UNPROTECT", layout: "audit", mission: "ROOT LOCK을 해제하고 최종 결과 저장", maxCalc: 15 },
  ],
};
